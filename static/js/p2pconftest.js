...
var sseconnecturl = "/newauth/api/joinConversation/";
var confpostmsgurl = "/newauth/api/addmessagetoconversation/";
...
function findconfusers() {
	var existingusersconnection1 = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
    existingusersconnection1.open('POST', "/newauth/api/getflakeconfusers/" + confflake + "/" + confusername, false);
    existingusersconnection1.withCredentials = true;
    existingusersconnection1.setRequestHeader('Content-Type', 'application/json');     
    //alert("getting conf user count");
	
    existingusersconnection1.onreadystatechange = function() {
    	if (existingusersconnection1.readyState == 4 && existingusersconnection1.status == 200) {
	           
        	var res = existingusersconnection1.responseText;  
        	//alert(res);
    ...
    existingusersconnection1.send(null);
...
function establishFlakeConfSSEConnection(callback, flake) {
	...
	if(typeof(EventSource) !== "undefined") {
		if (confsseeventsource == null) {
			...
			document.getElementById("flake-conf-updates").innerText = "Connected to conf SSE event source";
		    ...
		    // Message was sent by us
			 ...
			 // console.log('event data received ' + evt );
			  if (evt.data.length == 0 ){
				document.getElementById("flake-conf-updates").innerText += 'NO data in message event '  ;
				 return;
			  }
		   ...
		   // }
		     ...
		     // console.log('event data received ' + JSON.stringify(message) );
		      ...
		      if (typeof message.message !== 'undefined' && typeof message.message.type !== 'undefined' && message.message.type === 'user_left') {
		    	  document.getElementById("flake-conf-updates").innerText += 'message received ' + JSON.stringify(message.message) ;
		      }
		      
		    if (message.sdp) {
		    	document.getElementById("flake-conf-updates").innerText += 'sdp event data received ' + JSON.stringify(message.sdp) ;
		     
		    } else if (message.candidate) {
		      	document.getElementById("flake-conf-updates").innerText += 'candidate received ' + JSON.stringify(message.candidate) ;
		    }
		  });
...
// Send signaling data 
function sendMessage(mess, convid, confuser, flake) {
  
  	confpostmsgconnection = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
	confpostmsgconnection.open('POST', "/newauth/api/addmessagetoconversation/", false);
	confpostmsgconnection.withCredentials = true;
	confpostmsgconnection.setRequestHeader('Content-Type', 'application/json');      
   
   var reqpacket;
   if (typeof flake === 'undefined' || flake.length == 0) {
     reqpacket = JSON.stringify({
					 	   	sender : { phones : [confuser] },
					 	   message : mess,
					 	   sdp: "sdp here",
