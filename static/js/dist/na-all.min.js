...
f.classList.add("row");var g=document.createElement("div");g.classList.add("col-sm-8");g.classList.add("col-sm-offset-2");var h=document.createElement("p");h.classList.add("text-muted");h.setAttribute("id","createuser-flake-text");h.appendChild(document.createTextNode(" Click 'No', if you want to keep your selected location. Click 'I will choose a different spot' if you want to choose a new location."));h.style.color="#878787";h.style.fontWeight="600";g.appendChild(h);f.appendChild(g);e.appendChild(f);
f=document.createElement("div");f.classList.add("row");f.classList.add("text-center");g=document.createElement("div");g.setAttribute("id","cu-flake-can");f.appendChild(g);e.appendChild(f);f=document.createElement("div");f.classList.add("panel-body");f.classList.add("panel-word-wrap");g=document.createElement("div");g.classList.add("row");f.appendChild(g);var k=document.createElement("input");k.setAttribute("id","clr-box-instructions");k.classList.add("btn");k.classList.add("btn-primary");k.classList.add("btn-md");
...
let sidecontainer,totalViews=0,totalComments=0,totalRecords=0;const sqrt2=Math.sqrt(2);let currentPage,maxPage,minPage=0,scrollTimer,SSEerrorCount=0;const SSEmaxErrors=5;
...
function cleanString(a){if("undefined"!=typeof a)return a.replace(/[^\x20-\x7E\n]/g,"").replace(/[?]/g,"").replace(/\\+/g,"\\").replace(/\.+/g,".").replace(/^[^a-zA-Z0-9]+|[^a-zA-Z0-9]+$/g,"").replace(/\s[^a-zA-Z0-9]+\s/g," ")}
function topicdocumentclick(a){"undefined"!=typeof a&&(console.log("document click fired"),a.stopPropagation(),null!=document.getElementById("input-search-post")&&"none"!=document.getElementById("input-search-post").style.display&&"input-search-post"!=a.target.id&&(document.getElementById("input-search-post").style.display="none"));a=100;null!=clickstarttime&&(a=new Date-clickstarttime);if(100>=a){a=document.getElementsByClassName("dot-popup");if(null!=a)for(var b=a.length;0<a.length&&0<b;)a[0].classList.remove("expand-open"),
setTimeout(function(){},400),b--;a=document.getElementsByClassName("dot-desc");if(null!=a)for(null!=document.getElementById("showdescbtn")&&(document.getElementById("showdescbtn").onclick=function(a){displayallpostdescription(event)}),b=a.length;0<a.length&&0<b;)removediv(a[0]),b--;b=document.getElementsByClassName("expanded-dot");if(null!=b&&0<b.length)for(a=0;a<b.length;a++)shrinkdot(b[a])}else setTimeout(function(){clickstarttime=null},1E3)}
...
expandDot(document.getElementById("dot-0")),await n(2500),shrinkdot(document.getElementById("dot-0")));await n(300)})()}}}(e,l,h),50);l.style.opacity=window.getComputedStyle(l).getPropertyValue("opacity");l.style.transform=window.getComputedStyle(l).getPropertyValue("transform");k=0;"undefined"!=typeof a[h].week&&null!=a[h].week&&0<a[h].week.length&&(k=parseInt(moment().week())-parseInt(a[h].week));10<a.length&&0!=k?0<k?(l.style.opacity=Math.max(.04,1-.15*k),console.log("setting lower opacity for weekdiff "+
k+" "+(1-.15*k))):(l.style.opacity=Math.max(.04,1-.1*(52+k)),console.log("setting lower opacity for weekdiff "+k+" "+(1-.15*(52+k)))):l.style.opacity=1}}"undefined"!=typeof c&&0<c.length&&0==f&&displaybuttonbehavior(null,"No match found")}console.log("total unique values in hashset "+setofhashes.size);console.log("dot sizes",dotsizemap)}
...
d,e,f,g,!0)}};"undefined"==typeof a.crtime&&"undefined"!=typeof a.crTime&&(a.crtime=a.crTime);if(a.crtime){var k="/newauth/api/getpostsummarybytopicandtime/"+btoa(a.topic)+"/"+a.crtime;console.log("calling uriencoded url "+k);h.open("GET",k,!1);h.withCredentials=!0;h.setRequestHeader("Content-Type","application/json");h.send(null)}else console.warn("Post without crtime. Can not load summary "+JSON.stringify(a))}
...
function addcopylinktopopup(a){if(a)if(a.querySelector(".copy-icon"))console.log("Copy Icon Already exists for dot ");else{var b=document.createElement("span");b.className="copy-icon";if(!a.querySelector('.img-icon[src\x3d"/static/icons/copy-96.png"]')){var c=document.createElement("img");c.className="img-icon";c.src="/static/icons/copy-96.png";c.style.width="30px";c.style.height="30px";c.style.objectFit="contain";c.style.display="block";b.appendChild(c);c.title="Copy the dot"}Object.assign(b.style,
{margin:"2px",padding:"4px",color:"rgb(221, 221, 221)",cursor:"pointer",position:"absolute",width:"30px",height:"30px",fontSize:"18px",zIndex:"7",display:"none",alignItems:"center",justifyContent:"center",top:"10px",right:"10px"});a.querySelector(".open-icon")?(c=parseInt(a.querySelector(".open-icon").style.right||"10",10),console.log("right value of open icon "+c),b.style.right=`${c+24}px`):b.style.right="10px";b.addEventListener("click",b=>{b.stopPropagation();let c=a.id.split("-").pop(),d=posts[parseInt(c)];
console.log("Copyig post "+JSON.stringify(d));console.log("text "+d.data.text);isdotafolder(d.tags)?(document.getElementById("post-to-topic-select").style.display="none",document.getElementById("post-topic-label").innerText="Copy existing folder"):(document.getElementById("create-sub-topic-select").style.display="none",document.getElementById("post-topic-label").innerText="Copy existing post",document.getElementById("post-text-input").value=d.data.text,document.getElementById("post-text-input").addEventListener("input",
()=>{document.getElementById("contentpostbtn").disabled=!1}),document.getElementById("contentpostbtn").disabled=!0);posttotopicmodal(b,!0,c)});a.appendChild(b)}}
...
function posttotopicmodal(a,b,c){null!=a&&a.stopPropagation();if(null!=splitschemas&&0<splitschemas.length)if(document.getElementById("topic-post-data-input-holder").innerHTML="",1<splitschemas.length){document.getElementById("create-sub-topic-select").style.visibility="hidden";var d=topiccurrentdepth==splitschemas.length-1;document.getElementById("topic-post-data-input-holder").appendChild(generateformbasedonschema(splitschemas[topiccurrentdepth],d))}else document.getElementById("topic-post-data-input-holder").appendChild(generateformbasedonschema(splitschemas[0],
!0));else topiccurrentdepth>=topicmaxdepth?document.getElementById("create-sub-topic-select").style.visibility="hidden":document.getElementById("create-sub-topic-select").style.visibility="visible";if("undefined"!==typeof b&&!0===b){b=$("#topic-post-modal");d=b.find(".modal-dialog");b.css({position:"fixed",top:"-10000px",left:"-10000px",display:"block",visibility:"hidden"});const e=d.outerWidth(),f=d.outerHeight();b.css({position:"",top:"",left:"",display:"",visibility:""});const g=$(window).width(),
h=$(window).height();let k=a.clientX;a=a.clientY;"undefined"!=typeof c&&null!=c&&(c=document.getElementById("dot-"+c),c=parseInt(window.getComputedStyle(c).width,10)||0,k-=c);k=Math.max(10,Math.min(k,g-e-10));a=Math.max(10,Math.min(a,h-f-10));d.css({position:"fixed",top:a+"px",left:k+"px",margin:0,right:"auto",bottom:"auto"});b.modal({keyboard:!0})}else $("#topic-post-modal").find(".modal-dialog").css({position:"",top:"",left:"",margin:"",right:"",bottom:""}).end().modal("show");return!1}
...
function openSSEConnection(){"undefined"!==typeof EventSource?(topiceventsource=new EventSource("/newauth/api/monitorTopic/"+topicname),topiceventsource.addEventListener("message",function(a){try{if(a&&"string"===typeof a.data){try{var b=JSON.parse("["+a.data+"]")}catch(h){console.error("Failed to parse event.data:",h,a.data);return}if(Array.isArray(b)&&b[0]){var c=b[0];if(Array.isArray(posts)&&posts[0]){var d=posts[0],e=d.crtime?new Date(d.crtime):null,f=c.crTime?new Date(c.crTime):null;console.log("post received created at time",
c.crTime,d.crtime);SSEerrorCount=0;var g=e instanceof Date&&!isNaN(e)&&f instanceof Date&&!isNaN(f);if(c.comments&&!isNaN(parseInt(c.comments))&&0<parseInt(c.comments)&&null==c.summary)showpostcomment(c,!0);else if(!g||f>e)c.crtime=c.crTime||c.crtime,displaypostsasdots(b,!0)}else console.error("posts array is invalid:",posts)}else console.error("Parsed recdpost is invalid:",b)}else console.error("Invalid event.data",a)}catch(h){SSEerrorCount++,console.error("Unexpected error in post handler:",h)}}),
topiceventsource.onerror=a=>{SSEerrorCount++;console.warn("EventSource error",a,"count:",SSEerrorCount);SSEerrorCount>=SSEmaxErrors&&(console.error("Max errors reached. Closing EventSource."),closeSSEConnection())}):alert("This browser does not support the technology required to monitor the topic in realtime. You can refresh the page periodically to see the latest posts.")}function displayflakeconfstartmodal(a){$("#flake-conf-start-modal").modal("show")}
