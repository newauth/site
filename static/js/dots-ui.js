...
function createMouseOverHandler(div, obj) {
    return function() {
		...
		hoverTimers[obj.id] = setTimeout(function() {
		    ...
		    // ✅ Extract data BEFORE saving state and expanding
		    var popup = dotElement.querySelector('.dot-popup');
		    var items = popup ? popup.querySelectorAll('li') : [];
		    ...
		    PriceMoves.smartExpand(dotElement, function() {
		        // ✅ Pass pre-extracted data to showBasicPopup
		        PriceMoves.showBasicPopup(dotElement, ticker, price, time, exchange, bg);
		    });
...
function displaysidepanel(input) {
	...
	if (!document.getElementById('sp-refresh-btn')) {
		...
		var weekoffset = parseInt(window.location.pathname.split('/').pop()) || 0;
		...
		refreshBtn.addEventListener('click', function() {
		    ...
		    var url = '/newauth/api/refresh-earnings?weekOffset=' + weekoffset;
    ...
    objectList.forEach(function(obj, index) {
		...
		var reportTime = postData2.Time || '';
		var isBMO = reportTime.indexOf('BMO') > -1;    
		
		//console.log('' + JSON.stringify(obj.data) + ' | ' + reportTime + ' | ' + isBMO);   
