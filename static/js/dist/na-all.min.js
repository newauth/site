	...
	if (inposts != null) {
		...
		for (var i = 0; i < inposts.length; i++) {
			...
			if (typeof filter != 'undefined' && filter.length > 0) {
				...
				if (sortfield != null && sortvalues.length > 1 && typeof inposts[i].data != 'undefined' && typeof inposts[i].data[sortfield] != 'undefined') {
		...
		linkArray.forEach(text => {
			...
			linkItem.addEventListener("click", (event) => {
				...
				if (isActive) {
					...
					filterDivsByDataAttribute('#object-list-sidecontainer', sortfield, text, 'exact', true);
				}
			});
	...
	objectList.forEach((obj, index) => {
		...
		function createMouseOutHandler(div, obj) {
		    return function() {
		        ...
		        // ✅ If this dot is pinned (clicked), ignore mouseout completely
		        if (pinnedDotId === obj.id) {
		            return;
		        }

		        // Only shrink if not pinned
		        var dotElement = document.getElementById('dot-' + obj.id);
		        if (dotElement) {
		            PriceMoves.restoreDotState(dotElement, 'dot-' + obj.id);
		            if (typeof shrinkdot === 'function') {
		...
		function createClickHandler(div, obj) {
		    return function(e) {
		        ...
		        // Clear hover timer since click takes over
		        if (hoverTimers[obj.id]) {
		            clearTimeout(hoverTimers[obj.id]);
		            delete hoverTimers[obj.id];
		        }

		        // ✅ If clicking the already pinned dot, close it (toggle behavior)
		        if (pinnedDotId === obj.id) {
		            PriceMoves.closePinnedDot();
		            return;
		        }

		        // Close previously pinned dot if exists
		        if (pinnedDotId) {
		            PriceMoves.closePinnedDot();
		        }

		        // Ensure dot is expanded first
		        expandDot(dotElement, dotElement.getBoundingClientRect());

		        // Fetch & render signal data
		        PriceMoves.signal(dotElement, obj);

		        // Pin this dot so mouseout won't close it
		        pinnedDotId = obj.id;
		    };
		}
...
function displayRadialText(textList, canvasEl, options) {
	...
	var topMargin = margin + 30; // just 30px extra on top — enough to prevent edge sticking
