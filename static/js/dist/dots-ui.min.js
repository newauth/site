...
expandDot(document.getElementById("dot-0")),await q(2500),shrinkdot(document.getElementById("dot-0")));await q(300)})()}}}(d,l,g),50);l.style.opacity=window.getComputedStyle(l).getPropertyValue("opacity");l.style.transform=window.getComputedStyle(l).getPropertyValue("transform");k=0;"undefined"!=typeof a[g].week&&null!=a[g].week&&0<a[g].week.length&&(k=parseInt(moment().week())-parseInt(a[g].week));10<a.length&&0!=k?0<k?(l.style.opacity=Math.max(.04,1-.15*k),console.log("setting lower opacity for weekdiff "+
k+" "+(1-.15*k))):(l.style.opacity=Math.max(.04,1-.1*(52+k)),console.log("setting lower opacity for weekdiff "+k+" "+(1-.15*(52+k)))):l.style.opacity=1}}"undefined"!=typeof c&&0<c.length&&0==e&&displaybuttonbehavior(null,"No match found")}console.log("total unique values in hashset "+setofhashes.size);console.log("dot sizes",dotsizemap)}
