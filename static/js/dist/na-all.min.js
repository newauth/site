					...
					dot.addEventListener('mouseleave', function(e, idx) {
						return function(e) {
							...
							if (doexpand) {
								...
								if (PriceMoves) PriceMoves.closePinnedDot();
							} else {
...
function displaysidepanel(input) {
	...
	//console.log('in displaysidepanel: ' + input.id + ' - ' + JSON.stringify(input));
	...
	function shrinkCanvasForPanel(show) {
	    ...
	    if (show) {
	        var canvasWidth = canvas.offsetWidth; // capture BEFORE resizing
	        var newWidth = canvasWidth - panelWidth;
	        var scale = newWidth / canvasWidth;

	        // Anchor left edge explicitly first
	        canvas.style.position = 'absolute';
	        canvas.style.left = '0';
	        canvas.style.right = panelWidth + 'px';
	        canvas.style.width = 'auto'; // let left+right define the width
	        canvas.style.transition = 'right 0.3s ease';

	        dots.forEach(function(dot) {
	            var currentLeft = parseFloat(dot.style.left);
	            if (!isNaN(currentLeft)) {
	                dot.dataset.originalLeft = dot.style.left;
	                dot.style.left = (currentLeft * scale) + 'px';
	            }
	        });
	...
	if (!summaryPanel) {
		...
		// After creating summaryPanel and knowing its height:
	...
	objectList.forEach((obj, index) => {
		...
		if (sortfield != null && sortfield.length > 0 && typeof obj.data[sortfield] != 'undefined')
			div.setAttribute('data-' + sortfield, obj.data[sortfield]);

		const getText = (obj) => {
			if (obj && typeof obj.text === 'string') return obj.text;
			if (obj && obj.data && typeof obj.data.text === 'string') return obj.data.text;
			return ''; // fallback
		};

		const textSpan = document.createElement('span');
		textSpan.textContent = getText(obj);
		...
		// Store pending timeouts per element to allow cleanup
		// ES5 debounce utility
		function debounce(func, wait, options) {
		    options = options || {};
		    var timeout;
		    return function() {
		...
		// Track active timers per object ID
		var hoverTimers = {};
		...
		function createMouseOverHandler(div, obj) {
		    return function() {
		        ...
		        // ✅ Use getPinnedId() consistently — not local pinnedDotId
		        ...
		        // Show hint on sidepanel row immediately
		        if (!div.querySelector('.signal-hint')) {
		            var hint = document.createElement('div');
		            hint.className = 'signal-hint';
		            hint.textContent = '⚡ Click for signal';
		            hint.style.cssText = 'font-size:10px;color:#3b82f6;font-weight:600;margin-top:4px;letter-spacing:.03em';
		        ...
		        hoverTimers[obj.id] = setTimeout(function() {
		            ...
		            PriceMoves.smartExpand(dotElement, function() {
		                // ✅ Only inject content here — remove duplicate below
		                dotElement.classList.add('dot-expanding');
		                setTimeout(function() {
		                    dotElement.classList.remove('dot-expanding');
		                }, 300);
		            ...
		            // ✅ REMOVED duplicate content injection that was here
		...
		function createMouseOutHandler(div, obj) {
		    return function() {
		        ...
		        if (dotElement.dataset.dotState === 'hovering') {
		            ...
		            // ✅ No animation — just restore instantly
		...
		function createClickHandler(div, obj) {
		    return function(e) {
		        ...
		        // Toggle — clicking pinned dot closes it
		        if (String(PriceMoves.getPinnedId()) === String(obj.id)) {
					console.log('toggle close - dotState before:', dotElement.dataset.dotState);
				    PriceMoves.closePinnedDot();
				    console.log('toggle close - dotState after:', dotElement.dataset.dotState);
				    return;
		        }

		        // Close any other pinned dot first
		        if (PriceMoves.getPinnedId() !== null) PriceMoves.closePinnedDot();

		        // Save state only if not already saved by hover
		        if (dotElement.dataset.dotState !== 'hovering') {
		            PriceMoves.saveDotState(dotElement, dotElement.id);
		        }
		...
		// Store on the element to easily remove later
		div._mouseOverHandler = overHandler;
		div._mouseOutHandler = outHandler;

		// Remove existing listeners before adding new ones (prevents stacking)
		div.removeEventListener('mouseenter', div._prevOverHandler);
		div.removeEventListener('mouseleave', div._prevOutHandler);

		// Attach fresh
		div.addEventListener('mouseenter', overHandler);
		div.addEventListener('mouseleave', outHandler);
		div.addEventListener('click', createClickHandler(div, obj));

		// Optional: Add cursor pointer to indicate clickability
		div.style.cursor = 'pointer';

		// Save for next cleanup cycle
		div._prevOverHandler = overHandler;
		div._prevOutHandler = outHandler;
